...
const GamingLaptops = () => {
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchesCategory = product.category === 'Gaming';
  ...
  return (
    <div className={`min-h-screen pb-16 ${isDarkMode ? 'bg-gray-900 text-gray-100' : 'bg-gray-50 text-gray-800'} relative overflow-hidden`}>
      ...
      {isDarkMode ? (
        <>
          <CircleDecoration className="bg-red-700 opacity-10 w-96 h-96 -top-40 -right-20 z-0 blur-lg" />
          <CircleDecoration className="bg-orange-700 opacity-10 w-80 h-80 bottom-1/4 -left-20 z-0 blur-lg" />
          <CircleDecoration className="bg-indigo-700 opacity-5 w-64 h-64 top-1/2 right-1/4 z-0 blur-lg" />
          <CircleDecoration className="bg-red-700 opacity-5 w-60 h-60 bottom-20 right-40 z-0 blur-md" />
        </>
      ) : (
        <>
          <CircleDecoration className="bg-red-500 opacity-10 w-96 h-96 -top-40 -right-20 z-0 blur-lg" />
          <CircleDecoration className="bg-orange-500 opacity-10 w-80 h-80 bottom-1/4 -left-20 z-0 blur-lg" />
          <CircleDecoration className="bg-indigo-400 opacity-5 w-64 h-64 top-1/2 right-1/4 z-0 blur-lg" />
          <CircleDecoration className="bg-red-400 opacity-5 w-60 h-60 bottom-20 right-40 z-0 blur-md" />
        </>
      )}

      {/* Enhanced Hero Section */}
      <section className="relative overflow-hidden">
        <div className="relative bg-gradient-to-r from-red-900 via-orange-800 to-red-900 py-24 px-4">
          {/* Hero background decorations */}
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden">
            <div className="absolute -top-32 -left-32 w-96 h-96 bg-orange-400 rounded-full filter blur-3xl opacity-30"></div>
            <div className="absolute top-1/3 -right-32 w-96 h-96 bg-red-400 rounded-full filter blur-3xl opacity-30"></div>
            <div className="absolute bottom-0 left-1/4 w-64 h-64 bg-yellow-300 rounded-full filter blur-3xl opacity-20"></div>
            <div className="absolute top-10 right-1/4 w-32 h-32 bg-red-400 rounded-full filter blur-2xl opacity-20"></div>
          </div>
          ...
          <div className="max-w-7xl mx-auto relative">
            <div className="md:flex md:items-center md:justify-between">
              <div className="text-center md:text-left md:w-1/2">
                <div className="inline-flex items-center px-4 py-1 rounded-full bg-red-700/80 text-red-100 text-sm font-medium mb-6 shadow-lg backdrop-blur-sm">
                  <FireIcon className="h-4 w-4 mr-2" />
                  Maximum Performance
                </div>
                <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold mb-6 text-white leading-tight">
                  Gaming <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-300 to-yellow-300">Laptops</span>
                </h1>
                <p className="text-lg mb-8 text-red-100 max-w-xl mx-auto md:mx-0">
                  Immerse yourself in gaming with powerful machines featuring top-tier GPUs, high refresh rates, and epic performance for an uncompromised gaming experience.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
                  <Link
                    to="/catalog"
                    className="bg-transparent border-2 border-red-300 text-white px-8 py-3 rounded-lg font-medium hover:bg-red-800/50 hover:border-red-200 transition duration-300 flex items-center justify-center backdrop-blur-sm"
                  >
                  ...
                  <Link
                    to="catalog"
                    className="bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-700 hover:to-orange-700 text-white px-8 py-3 rounded-lg font-medium transition duration-300 flex items-center justify-center shadow-lg"
                  >
              ...
              <div className="hidden md:block md:w-2/5">
                <div className="bg-white/10 backdrop-blur-md rounded-xl p-6 shadow-xl mt-8 md:mt-0 border border-white/20">
                  <h3 className="text-xl font-semibold text-white mb-4">Built for Gamers</h3>
                  <ul className="space-y-3">
                    {['High-performance GPUs', 'Fast refresh rate displays', 'Advanced cooling systems', 'RGB lighting options'].map((feature, index) => (
      ...
      <section id="products" className="px-4 pt-6 pb-12 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className={`p-6 rounded-xl shadow-lg mb-8 relative overflow-hidden ${isDarkMode ? 'bg-gray-800/80 backdrop-blur-md' : 'bg-white/80 backdrop-blur-md'} border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
            <div className="absolute top-0 right-0 w-40 h-40 bg-red-500 opacity-5 rounded-full transform translate-x-20 -translate-y-20"></div>
            <div className="absolute bottom-0 left-0 w-40 h-40 bg-orange-500 opacity-5 rounded-full transform -translate-x-20 translate-y-20"></div>
            <div className="relative mb-6">
              ...
              <input
                type="text"
                placeholder="Search gaming laptops..."
                className={`pl-12 pr-4 py-3 w-full rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 transition-all duration-300 ${
            ...
            <div className="flex flex-col md:flex-row justify-between gap-4">
              <div className="relative flex-grow">
                ...
                <select
                  ...
                  className={`w-full pl-4 pr-10 py-2.5 rounded-lg appearance-none focus:outline-none focus:ring-2 focus:ring-red-500 transition-all duration-300 ${
          ...
          <div className="flex justify-between items-center mb-8">
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <SparklesIcon className="h-6 w-6 text-red-500 relative" />
              </div>
              <h2 className="text-xl font-bold">{isLoading ? "Loading..." : `${sortedProducts.length} Gaming Laptops`}</h2>
            </div>
            ...
            {isLoading ? (
              ...
              <div className={`col-span-full p-10 text-center rounded-xl ${isDarkMode ? 'bg-gray-800/90 backdrop-blur-sm' : 'bg-white/90 backdrop-blur-sm'} shadow-md border ${isDarkMode ? 'border-gray-700/50' : 'border-gray-200/50'}`}>
                <div className="flex flex-col items-center">
                  ...
                  <h3 className="text-xl font-medium mb-2">No gaming laptops found</h3>
                  <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-500'} mb-4`}>
                    Try adjusting your search to find what you're looking for.
                  </p>
                  <button 
                    onClick={() => { setSearchTerm(''); setSortBy('featured'); }}
                    className={`px-4 py-2 rounded-lg transition-all duration-300 ${isDarkMode ? 'bg-red-600 hover:bg-red-700' : 'bg-red-500 hover:bg-red-600'} text-white`}
                  >
                    Clear Filters
                  </button>
                </div>
